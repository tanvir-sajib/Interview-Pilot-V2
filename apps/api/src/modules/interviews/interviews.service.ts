...
export class InterviewsService {
  ...
  async startSession(id: string) {
    ...
    if (session.status !== 'READY') throw new BadRequestException('Cannot start');
    return this.prisma.interviewSession.update({ where: { id }, data: { status: 'IN_PROGRESS', startedAt: new Date() } });
  }
  ...
  async submitAnswer(sessionId: string, questionId: string, content: string) {
    ...
    if (session.status !== 'IN_PROGRESS') throw new BadRequestException('Invalid session state');
  ...
  async completeSession(id: string) {
    const session = await this.prisma.interviewSession.findUnique({ where: { id } });
    if (!session) return null;
    if (session.status !== 'IN_PROGRESS') throw new BadRequestException('Invalid state');
    return this.prisma.interviewSession.update({ where: { id }, data: { status: 'COMPLETED', endedAt: new Date() } });
  }
  ...
  async cancelSession(id: string) {
    ...
    if (session.status === 'CANCELLED') throw new BadRequestException('Session already cancelled');
    return this.prisma.interviewSession.update({ where: { id }, data: { status: 'CANCELLED', endedAt: new Date() } });
  }
}
